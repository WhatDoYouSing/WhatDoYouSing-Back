...
class ProfileView(views.APIView):
    ...
    def get(self, request, format=None):
        ...
        if serializer.is_valid():
            return Response({'message': '마이페이지 조회 성공', 'data': serializer.validated_data}, status=status.HTTP_200_OK)
...
class EmotionsCollectView(views.APIView, PaginationHandlerMixin):
    ...
    def get(self, request):
        ...
        emotion_content = request.GET.get('emotion_content', None)
        myEmotions = Emotion.objects.filter(emo_user=request.user)
